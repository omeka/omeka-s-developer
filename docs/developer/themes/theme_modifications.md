...
Themes load their CSS and Javascript files using Laminas's view helper functions ([`headLink()`](https://docs.laminas.dev/laminas-view/v2/helpers/head-link/) and [`headScript()`](https://docs.laminas.dev/laminas-view/v2/helpers/head-script/)), which help control the order in which these files load alongside files from Omeka S's pages and  modules. These functions should appear before the `<head>` of layout.phtml.
